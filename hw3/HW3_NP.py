...
def inputFile():
    ...
    reward_board = numpy.empty((size_grid, size_grid,))
    reward_board[:] = -1
    print (reward_board)
    ...
    for i in xrange(no_of_obstacles):
        ...
        reward_board[int(x)][int(y)] = int(-101)
    ...
    return reward_board,size_grid,car_combined_coordinates,no_of_cars
...
def update_up_matrix(prev_utility_matrix,up_matrix):

    up_matrix = np.roll(prev_utility_matrix)
    up_matrix[0] = up_matrix[1]

    return up_matrix

def update_down_matrix(prev_utility_matrix,down_matrix):
    up_matrix = np.roll(prev_utility_matrix)
    up_matrix[0] = up_matrix[1]
...
def update_right_matrix(prev_utility_matrix,right_matrix):

def update_left_matrix(prev_utility_matrix,left_matrix):
...
for i in xrange(no_of_cars):
    ...
    up_matrix = numpy.empty((size_grid, size_grid,))
    down_matrix = numpy.empty((size_grid, size_grid,))
    right_matrix = numpy.empty((size_grid, size_grid,))
    left_matrix = numpy.empty((size_grid, size_grid,))
